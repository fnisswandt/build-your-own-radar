...
const Radar = function(size, radar) {
  ...
  function plotQuadrant(rings, quadrant) {
    var quadrantGroup = svg
      ...
      .on("click", selectQuadrant.bind({}, quadrant.order, quadrant.startAngle, "a"));
  ...
  function drawBlipInCoordinates(blip, coordinates, order, quadrantGroup, ringList) {
    ...
    blipListItem.attr("number", blip.number());
    ...
    var clickBlip = function() {
      var selectedId = this.attributes.number.value;
      clearSelected(selectedId);

      toggleEntry(selectedId);
    ...
    group.on("click", function() {
      ...
      var entry = toggleEntry(selectedId);

      if (entry) {
        scrollToBlip(entry.blipId, isZoomed);
      }
    });
  }

  // selecting! view-fourth-zoom-undefined view-second-zoom-undefined
  ...
  function plotQuadrantButtons(quadrants, header) {
    function addButton(quadrant) {
      ...
      header
        ...
        .on("mouseover", mouseoverQuadrant.bind({}, quadrant.order))
        .on("mouseout", mouseoutQuadrant.bind({}, quadrant.order))
        .on("click", selectQuadrant.bind({}, quadrant.order, quadrant.startAngle, "b"));
    }
    ...
    _.each([0, 1, 2, 3], function(i) {
      _.each(quadrants[i].quadrant.blips(), function(blip) {
        itemList.push({
          ...
          quadrantId: i,
          description: "" + blip.name() + " - " + blip.ring().name() + " (" + blip.number() + ")"
        });
      });
    ...
    header
      ...
      .text("Search")
      .on("click", function() {
        header.select("#search-box").attr("class", "search-box-visible");
      });

    var searchDiv = header
      .append("div")
      .attr("id", "search-box")
    ...
    var list = searchDiv
      ...
      .on("change", function() {
        ...
        selectQuadrant(current.order, current.startAngle, "c");

        clearSelected();
        var entry = toggleEntry(e.selectedIndex);

        scrollToBlip(entry.blipId);
      });
  ...
  function scrollToBlip(blipId, fast) {
    let ms = 1100;
    ...
    setTimeout(function() {
      ...
      document.getElementById(blipId).classList.add("highlight");
    }, ms);
  }

  let selectedEntryId = -1;

  function clearSelected(selectedId) {
    if (!selectedId) {
      selectedEntryId = -1;
    }

    d3.selectAll(".blip-list-item.highlight").classed("highlight", false);
    d3.selectAll(".blip-item-description.expanded").classed("expanded", false);
  }
  ...
  function maintainHighlight() {
    ...
    if (selectedEntryId) {
      var entry = getBlipInfoFromNumber(selectedEntryId);
      d3.selectAll("#" + entry.blipId).classed("highlight", true);
    }
    // }
  }

  function getBlipInfoFromNumber(id) {
    var entry = {};
    entry.blipId = "blip-item-" + id;
    entry.blipDescId = "blip-desc-" + id;
  ...
  function toggleEntry(id) {
    ...
    console.log("toggle called", id, isZoomed, viewState);
    ...
    selectedEntryId = id;
    var entry = getBlipInfoFromNumber(id);

    d3.select("#" + entry.blipId).classed("highlight", true);
    d3.select("#" + entry.blipDescId).classed("expanded", true);
  ...
  function plotRadarFooter() {
    d3.select("body")
      ...
      .append("p")
      .html(
        'This software is <a href="https://github.com/reedbusiness/build-your-own-radar">open source</a> and available for download and self-hosting.'
      );
  }
  ...
  function selectQuadrant(order, startAngle, which) {
    var newState = `view-${order}-zoom-${isZoomed}`;
    ...
    var radarWidthPercentage = 0.9;
    var moveRight = ((1 + adjustX) * (radarWidthPercentage * window.innerWidth - size)) / 2;
    var moveLeft = ((1 - adjustX) * (radarWidthPercentage * window.innerWidth - size)) / 2;
